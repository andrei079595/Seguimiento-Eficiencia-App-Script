    ...
    for (let i = 1; i < paramsData.length; i++) {
        ...
        if (rowEmail === userEmail.toLowerCase().trim()) {
            userProfile = {
                ...
                departamento: paramsData[i][4], 
                perfil: parseInt(paramsData[i][5]) || 3, 
                deptUsers: paramsData.filter(r => r[4] === paramsData[i][4]).map(r => r[0])
            };
            break;
        }
    }
...
function pruneEmptyRows(data) {
    ...
    return data.filter(row => row.some(cell => cell !== "" && cell !== null && cell !== undefined))
               .map(row => row.map(cell => {
                    if (cell === "" || cell === null || cell === undefined) return null;
                    if (cell instanceof Date) return isNaN(cell.getTime()) ? "Fecha Inválida" : cell.toISOString();
                    return cell;
               }));
}
...
function submitObservation(payload) {
    ...
    obsSheet.appendRow([
        ...
        payload.codigo || 'N/A',
        payload.nombre || Session.getActiveUser().getEmail(),
        payload.observacion || '',
        new Date()
    ]);
    return { success: true };
